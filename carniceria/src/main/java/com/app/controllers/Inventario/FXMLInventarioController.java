...
public class FXMLInventarioController implements Initializable {
    ...
    private void buscar(String consultaTexto) {
        ...
        if (consultaTexto == null || consultaTexto.isEmpty()) {
            ...
            // Iterar sobre la lista original y agregar los elementos que coincidan con el
            // filtro y la categoría seleccionada
            for (Productos producto : productosOriginalData) {
                String idString = producto.getId().toString();
                String nombreProducto = producto.getNombre(); // Suponiendo que tengas un método `getNombre()` en tu
                                                              // clase `Productos`

                // Verificar si tanto el ID como el nombre del producto contienen el texto de
                // consulta
                ...
                if ((idString.contains(consultaTexto) || nombreProducto.contains(consultaTexto))
                        && (selectedCategory == null || selectedCategory.equals("Todos")
                                || producto.getCategoria().getNombreCategoria().equals(selectedCategory))) {
    ...
    public void cargarCategorias(ComboBox<String> categorias, int opcion) {
        ...
        try {
            ...
            if (opcion == 1)
                listaNombresCategorias.add("Todos");
            listaNombresCategorias.addAll(query.list());
            ObservableList<String> observableListaNombresCategorias = FXCollections
                    .observableArrayList(listaNombresCategorias);
    ...
    public void Exportar() {
        ...
        String[] columnNames = { "ID", "Nombre", "Existencia", "Costo", "Precio Venta" }; // Agrega aquí los nombres de
                                                                                          // tus columnas
        int colNum = 0;
        ...
        String nombreArchivo = "Inventario "
                + (categorias.getValue() != null && !categorias.getValue().equalsIgnoreCase("Todos")
                        ? categorias.getValue()
                        : "Total")
                + " " + fechaExcell + ".xlsx";
    ...
    public void productosBajos() {
        ...
        // Realizar la consulta para obtener los productos con cantidad menor o igual al
        // inventario mínimo
